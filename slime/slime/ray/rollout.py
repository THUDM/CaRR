    ...
    for key in data.keys():
        ...
        for metric in (
            ...
            if metric in data[key]:
                scores = data[key][metric]
                if len(scores) > 0:
                    log_dict[f"eval/{key}-{metric}"] = sum(scores) / len(scores)

        if samples is not None:
            for metric, score in _compute_legacy_eval_metrics_from_samples(samples).items():
                log_dict.setdefault(f"eval/{key}-{metric}", score)
...
def _log_rollout_data(rollout_id, args, samples, rollout_extra_metrics, rollout_time):
    ...
    log_dict |= dict_add_prefix(_compute_source_split_metrics(args, samples), "rollout/")
    log_dict |= dict_add_prefix(_compute_group_reward_distribution(args, samples), "rollout/")
...
def compute_metrics_from_samples(args, samples):
    ...
    log_dict |= _compute_metadata_metrics_from_samples(samples)
...
def _compute_metadata_metrics_from_samples(samples: list[Sample]) -> dict[str, float]:
    if len(samples) == 0:
        return {}

    metrics: dict[str, float] = {
        "response_length": float(np.mean([sample.response_length for sample in samples]).item()),
        "unmaksed_sample_ratio": float(
            np.mean([int(sample.loss_mask is None or sum(sample.loss_mask) > 0) for sample in samples]).item()
        ),
    }
    ...
    for key in metadata_keys:
        values: list[float] = []
        for sample in samples:
            if isinstance(sample.metadata, dict) and key in sample.metadata:
                values.append(sample.metadata[key])
        if len(values) > 0:
            metrics[key] = float(np.mean(values).item())

    return metrics


def _compute_legacy_eval_metrics_from_samples(samples: list[Sample]) -> dict[str, float]:
    if len(samples) == 0:
        return {}

    metrics: dict[str, float] = {
        "response_length": float(np.mean([sample.response_length for sample in samples]).item()),
        "truncated": float(np.mean([int(sample.status == Sample.Status.TRUNCATED) for sample in samples]).item()),
    }
...
def _compute_source_split_metrics(args, samples: list[Sample]) -> dict[str, float]:
    if len(samples) == 0:
        return {}

    source_to_samples = group_by(samples, lambda sample: sample.source or "unknown")
    metrics: dict[str, float] = {}

    for source, source_samples in source_to_samples.items():
        source_metrics: dict[str, float] = {
            "response_lengths": float(np.mean([sample.response_length for sample in source_samples]).item()),
            "raw_reward": float(np.mean([sample.get_reward_value(args) for sample in source_samples]).item()),
            "truncated": float(
                np.mean([int(sample.status == Sample.Status.TRUNCATED) for sample in source_samples]).item()
            ),
            "unmaksed_sample_ratio": float(
                np.mean([int(sample.loss_mask is None or sum(sample.loss_mask) > 0) for sample in source_samples]).item()
            ),
        }

        loss_token_num: list[float] = []
        for sample in source_samples:
            if isinstance(sample.metadata, dict) and "loss_token_num" in sample.metadata:
                loss_token_num.append(sample.metadata["loss_token_num"])
            else:
                loss_token_num.append(sum(sample.loss_mask) if sample.loss_mask is not None else sample.response_length)
        source_metrics["loss_token_num"] = float(np.mean(loss_token_num).item())
        ...
        for key in (
            ...
            for sample in source_samples:
                if isinstance(sample.metadata, dict) and key in sample.metadata:
                    values.append(sample.metadata[key])
            if len(values) > 0:
                source_metrics[key] = float(np.mean(values).item())

        for key, value in source_metrics.items():
            metrics[f"{source}/{key}"] = value

    return metrics


def _compute_group_reward_distribution(args, samples: list[Sample]) -> dict[str, float]:
    if len(samples) == 0 or not hasattr(args, "n_samples_per_prompt"):
        return {}

    grouped_samples = group_by(samples, lambda sample: sample.group_index)
    valid_groups = [group for group in grouped_samples.values() if group and group[0].group_index is not None]
    ...
    all_one_groups = 0
    all_zero_groups = 0
    non_all_zero_groups = 0

    for group in valid_groups:
        group_rewards = [sample.get_reward_value(args) for sample in group]
        if all(abs(reward - 1.0) < 1e-6 for reward in group_rewards):
            all_one_groups += 1
        elif all(abs(reward) < 1e-6 for reward in group_rewards):
            all_zero_groups += 1
        else:
            non_all_zero_groups += 1
    ...
    return {
        "group_all_one_ratio": all_one_groups / total_groups,
        "group_non_all_zero_ratio": non_all_zero_groups / total_groups,
        "group_all_zero_ratio": all_zero_groups / total_groups,
    }
