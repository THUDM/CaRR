...
async def generate_and_rm(
    ...
    if isinstance(sample, list):
        ...
        if sample.reward is None:
            ...
            if isinstance(remote_env_info, dict) and "rubric_reward_ratio" not in remote_env_info:
                remote_env_info["rubric_reward_ratio"] = getattr(args, "rubric_reward_ratio", 0) if not evaluation else 0
            reward_result = await async_rm(args, sample)
            if isinstance(reward_result, dict):
                sample.reward = reward_result.get("reward", 0)
                if "outcome_reward" in reward_result:
                    metadata["outcome_reward"] = reward_result["outcome_reward"]
                if "rubric_reward" in reward_result:
                    metadata["rubric_reward"] = reward_result["rubric_reward"]
                if "rubric_scores" in reward_result:
                    metadata["rubric_scores"] = reward_result.get("rubric_scores", {})
            else:
...
async def generate_and_rm_group(
    ...
    if group and isinstance(group[0].metadata, dict) and "rubric_reward" in group[0].metadata:
        if getattr(args, "use_rubric_reward_nobfs", False):
            for sample in group:
                rubric_scores = sample.metadata.get("rubric_scores", {})
                if len(rubric_scores) == 0:
                    sample.metadata["rubric_reward"] = 0
                else:
                    sample.metadata["rubric_reward"] = sum(x.get("is_supported", 0) for x in rubric_scores.values()) / len(
                        rubric_scores
                    )

        if getattr(args, "normalize_rubric_reward", False):
            max_rubric_reward = max(sample.metadata.get("rubric_reward", 0) for sample in group)
            for sample in group:
                outcome_reward = sample.metadata.get("outcome_reward", 0)
                rubric_reward = sample.metadata.get("rubric_reward", 0)
                normalized_rubric_reward = rubric_reward / max_rubric_reward if max_rubric_reward > 0 else rubric_reward
                if getattr(args, "rubric_only_negative", False):
                    sample.reward = (
                        outcome_reward if outcome_reward > 0 else (args.rubric_reward_ratio * normalized_rubric_reward)
                    )
                elif getattr(args, "rubric_only_positive", False):
                    sample.reward = (
                        outcome_reward
                        if outcome_reward == 0
                        else ((1 - args.rubric_reward_ratio) * outcome_reward + args.rubric_reward_ratio * rubric_reward)
                    )
                else:
                    sample.reward = (1 - args.rubric_reward_ratio) * outcome_reward + args.rubric_reward_ratio * normalized_rubric_reward
        else:
            for sample in group:
                outcome_reward = sample.metadata.get("outcome_reward", 0)
                rubric_reward = sample.metadata.get("rubric_reward", 0)
                if getattr(args, "rubric_only_negative", False):
                    sample.reward = outcome_reward if outcome_reward > 0 else (args.rubric_reward_ratio * rubric_reward)
                elif getattr(args, "rubric_only_positive", False):
                    sample.reward = (
                        outcome_reward
                        if outcome_reward == 0
                    ...
                    sample.reward = (1 - args.rubric_reward_ratio) * outcome_reward + args.rubric_reward_ratio * rubric_reward
...
async def eval_rollout_single_dataset(
    ...
    if cache_key not in EVAL_PROMPT_DATASET:
        ...
        EVAL_PROMPT_DATASET[cache_key] = Dataset(
            ...
            source_key=getattr(dataset_cfg, "source_key", None),
            source_config_path=getattr(args, "source_config_path", None),
            remote_env_config_path=getattr(args, "remote_env_config_path", None),
        )
    ...
    for _i, prompt_sample in enumerate(dataset.samples):
        for j in range(dataset_cfg.n_samples_per_eval_prompt):
            ...
            sample.metadata.setdefault("session_id", str(uuid.uuid4()))
