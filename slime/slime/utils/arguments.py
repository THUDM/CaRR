...
def get_slime_extra_args_provider(add_custom_arguments=None):
    def add_slime_arguments(parser):
        ...
        def add_eval_arguments(parser):
            ...
            parser.add_argument("--eval-source-key", type=str, default=None, help="JSON dataset key")
            parser.add_argument("--eval-metadata-key", type=str, default=None, help="JSON dataset key")
            parser.add_argument(
            ...
            parser.add_argument(
                "--eval-results-save-dir",
                type=str,
                default=None,
                help="The directory to save detailed evaluation results for each evaluation dataset.",
            )
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=float,
                default=0,
                help="Ratio of rubric reward",
            )
            parser.add_argument(
                "--normalize-rubric-reward",
                action="store_true",
                default=False,
                help="Whether to normalize rubric reward in group.",
            )
            parser.add_argument(
                "--rubric-only-negative",
                action="store_true",
                default=False,
                help="If set, only negative samples will use rubric reward.",
            )
            parser.add_argument(
                "--rubric-only-positive",
                action="store_true",
                default=False,
                help="If set, only positive samples will use rubric reward.",
            )
            parser.add_argument(
                "--use-rubric-at-least-two-finished",
                action="store_true",
                default=False,
                help="If set, only use rubric reward when there are at least two finished rollouts in a group.",
            )
            parser.add_argument(
                "--use-rubric-reward-nobfs",
                action="store_true",
                default=False,
                help="Not use bfs from answer when calculating rubric score",
            )
            parser.add_argument(
                "--advantage-estimator",
                type=str,
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                "--no-punish-for-illform",
                action="store_true",
                help=("If not set, ill-formed responses will receive 0 reward"),
            )
            parser.add_argument(
                "--stop-once-illform",
                action="store_true",
                help=("If not set, rollout will stop once producing ill-formed responses"),
            )
            parser.add_argument(
        ...
        def add_rollout_buffer_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=int,
                default=60,
                help="Max retry num of running a tool in rollout.",
            )
            parser.add_argument(
                "--tool-timeout",
                type=int,
                default=60,
                help="Timeout limit (second) of running a tool in rollout.",
            )
            parser.add_argument(
            ...
            parser.add_argument("--source-key", type=str, default=None, help="JSON dataset key")
            parser.add_argument(
                "--source-config-path",
                type=str,
                default="configs/source_config.json",
                help=("Configs of reward model and remote environment for each data source."),
            )
            parser.add_argument(
                "--remote-env-config-path",
                type=str,
                default="configs/remote_env_config.json",
                help=("Configs for each remote environment."),
            )
            return parser
