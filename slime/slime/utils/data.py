...
class Dataset:
    def __init__(
        ...
        *,
        ...
        remote_env_config_path=None,
    ):
        origin_samples = []
        for data in read_file(path):
            if source_key and source_key in data:
                from slime.rollout.message_processor_hub import get_message_processor

                if source_config_path is None:
                    raise ValueError("source_config_path is required when source_key is used.")
                source_config = json.load(open(source_config_path, "r", encoding="utf-8"))

                source = data[source_key]
                if source not in source_config:
                    raise AssertionError(f'Data source "{source}" is not defined in {source_config_path}')

                input_messages = data[prompt_key]
                label = data.get(label_key, None)
                tools = data.get(tool_key, []) if tool_key is not None else []
                metadata = data.get(metadata_key) or {}
                metadata["input_messages"] = input_messages

                message_processor = get_message_processor(
                    processor_class_name=source_config[source]["message_processor"]["class"],
                    tokenizer=tokenizer,
                    **source_config[source]["message_processor"].get("kwargs", {}),
                )
                prompt = message_processor.apply_chat_template(input_messages, tools=tools)
                ...
                reward_model_info = deepcopy(source_config[source].get("reward_model", {}))
                if reward_model_info:
                    metadata["reward_model_info"] = reward_model_info

                remote_env_info = deepcopy(source_config[source].get("remote_environment", None))
                if remote_env_info is not None:
                    if "launch_env_commond" in remote_env_info:
                        del remote_env_info["launch_env_commond"]
                    if "remote_env_info" in metadata and isinstance(metadata["remote_env_info"], dict):
                        remote_env_info.update(metadata["remote_env_info"])
